...
fn main() {
    ...
    let mut cells = cell_ops::initialize_domain(num_cells, length, velocity);

    // Compute the initial total mass
    let initial_mass = cell_ops::Cell::total_mass(&cells);
    let initial_momentum = cell_ops::Cell::total_momentum(&cells);
    ...
    // Run the time loop
    time_stepping_mod::base::time_loop(&mut cells, velocity, dt, total_time, initial_mass, initial_momentum);

    // Compute the final total mass
    let final_mass = cell_ops::Cell::total_mass(&cells);
    let final_momentum = cell_ops::Cell::total_momentum(&cells);
