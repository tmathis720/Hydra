use crate::domain::{DPoint, DPointType, Section};
use std::fs::File;
...
impl GmshParser {
    ...
    pub fn load_mesh(file_path: &str) -> Result<(Vec<DPoint>, Vec<DPoint>), io::Error> {
        ...
        for line in reader.lines() {
            ...
            if in_nodes_section && current_node_line <= node_count {
                points.push(Self::parse_node(&line)?);
                current_node_line += 1;
            }
        ...
        Ok((points, elements)) // Return the parsed data; face handling can be added later
    }

    /// Parse a single node from a line in the Gmsh file
    fn parse_node(line: &str) -> Result<DPoint, io::Error> {
        ...
        Ok(DPoint::new(id, DPointType::Vertex, 3))  // Example assumes 3 fields per node
    }

    /// Parse an element from a line in the Gmsh file
    fn parse_element(line: &str) -> Result<DPoint, io::Error> {
        let mut split = line.split_whitespace();
        ...
        Ok(DPoint::new(id, DPointType::Cell, node_ids.len()))
    }
...
mod tests {
    ...
    fn test_circle_mesh_import() {
        ...
        let (nodes, elements) = result.unwrap();
