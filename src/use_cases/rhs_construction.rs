...
mod tests {
    ...
    fn test_build_zero_rhs() {
        ...
        for i in 0..size {
            assert_eq!(rhs[(i, 0)], 0.0, "RHS vector should be initialized to zero.");
        }
    }
    ...
    fn test_resize_rhs() {
        ...
        assert_eq!(rhs[(0, 0)], 1.0, "Original data should be preserved.");
        assert_eq!(rhs[(1, 0)], 2.0, "Original data should be preserved.");
        assert_eq!(rhs[(2, 0)], 3.0, "Original data should be preserved.");
        assert_eq!(rhs[(3, 0)], 0.0, "New elements should be initialized to zero.");
        assert_eq!(rhs[(4, 0)], 0.0, "New elements should be initialized to zero.");
    }
}
